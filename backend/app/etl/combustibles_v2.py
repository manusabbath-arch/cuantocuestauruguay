...
class CombustiblesETLv2(ETLBase):
    ...
    def transform(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not fecha_col:
            raise ValueError(
                f"No se encontró columna de fecha. Columnas: {data.columns.tolist()}"
            )
        ...
        if before_count > after_count:
            self.logger.warning(
                f"Eliminadas {before_count - after_count} filas con fechas inválidas"
            )
    ...
    def load(self, data: pd.DataFrame) -> None:
            ...
            try:
                ...
                for key, nombre in self.PRODUCTOS_MAP.items():
                    if key.lower() in str(row).lower():
                        producto = self.db_session.query(Producto).filter(
                            Producto.nombre == nombre
                        ).first()
                        
                        if producto and 'fecha' in row and pd.notna(row['fecha']):
                            # Verificar si ya existe
                            existing = self.db_session.query(Precio).filter(
                                Precio.producto_id == producto.id,
                                Precio.fecha == row['fecha']
                            ).first()
        ...
        self.logger.info(
            f"Carga completada: {loaded_count} insertados, {skipped_count} duplicados"
        )
