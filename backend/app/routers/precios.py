...
from app.models.schemas import (
    ProductoResponse, PrecioResponse, PrecioConProducto,
...
async def obtener_ultimo_precio(
    producto_id: int,
    db: Session = Depends(get_db)
):
    ...
    ).order_by(desc(Precio.fecha)).first()
...
async def calcular_variacion(
    ...
    precio_actual = db.query(Precio).filter(
        Precio.producto_id == producto_id
    ).order_by(desc(Precio.fecha)).first()
    ...
    precio_anterior = db.query(Precio).filter(
        Precio.producto_id == producto_id,
        Precio.fecha <= fecha_referencia
    ).order_by(desc(Precio.fecha)).first()
    
    if not precio_anterior:
        raise HTTPException(
            status_code=404,
            detail=f"No hay datos históricos suficientes para calcular variación de {periodo}"
        )
...
async def comparar_productos(
    ...
    datos = [
        ComparacionItem(fecha=fecha, valores=valores)
        for fecha, valores in sorted(datos_por_fecha.items())
    ]
